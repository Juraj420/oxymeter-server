...
app.post("/api/assign-device", (req, res) => {
  ...
  if (!device_uid || device_uid.trim() === "") {
    return res.status(400).json({ success: false, message: "Chýba Device UID" });
  }
  ...
  db.query("SELECT * FROM devices WHERE device_uid = ?", [device_uid.trim()], (err, results) => {
    ...
    if (results.length === 0) {
      ...
      db.query(
        ...
        err2 => {
          if (err2) return res.status(500).json({ success: false, message: "Chyba pri vytváraní zariadenia" });
          res.json({ success: true, message: "Zariadenie vytvorené a priradené k vášmu účtu" });
        }
      );
      ...
      db.query(
        ...
        err2 => {
          ...
          res.json({ success: true, message: "Zariadenie úspešne priradené k vášmu účtu" });
        }
      );
    }
  });
